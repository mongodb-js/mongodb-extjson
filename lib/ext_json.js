  ...
  if (module) {
    for (var i = 0; i < BSONTypes.length; i++) {
      if (!module[BSONTypes[i]])
        throw new Error('passed in module does not contain all BSON types required');
    }
...
ExtJSON.extend = function(module) {
  if (!module)
    throw new Error(
      "expecting mongodb module, invoke by calling ExtJSON.extend(require('mongodb'))"
    );
...
function deseralizeValue(self, value, options) {
  ...
  if (value['$binary'] != null) {
    ...
    type = value['$type'] ? parseInt(value['$type'], 16) : 0;
    return new self.bson.Binary(data, type);
  }
  ...
  if (value['$code'] != null) {
    return new self.bson.Code(
      value['$code'],
      deseralizeValue(self, value['$scope'] || {}, options)
    );
  }
  ...
  if (value['$ref'] != null) {
    return new self.bson.DBRef(
      value['$ref'],
      deseralizeValue(self, value['$id'], options),
      value['$db']
    );
  }
...
function serializeDocument(doc) {
  ...
  for (var name in doc) {
    if (Array.isArray(doc[name])) {
      ...
      doc[name] != null &&
      doc[name]._bsontype &&
      BSONTypes.indexOf(doc[name]._bsontype) !== -1
    ) {
      _doc[name] = doc[name];
